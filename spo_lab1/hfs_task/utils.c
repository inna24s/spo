...
HFSPlus *getHFSPlus(char *name) {
    ...
    //pread() читает максимум count байтов из описателя файлов fd, начиная со смещения offset (от начала файла), в буфер buf. Текущая позиция файла не изменяется.
...
Node *getNode(int nodeNumber, BTree *tree) {
    ...
    //выделяем память под node и дескриптор
...
char *recursive_copy(HFSPlus *hfs, Record *record, char *dest, char *message) {
    ...
    if (record->type == kHFSPlusFileRecord) {
        ...
        //если не удалось лткрыть место назначения
        if (fd == -1) {
